...
{
    [TestFixture]
    public class OnResourcesGetDataShould
    {
        [Test]
        public void AddResourcesToViewModel_When_OnResourcesGetDataEventIsRaised()
        {
            // Arrange
            var viewMock = new Mock<IResourcesView>();
            viewMock.Setup(v => v.Model).Returns(new ResourceViewModel());

            var resources = this.GetCategoriesWithResources();
            var resourceServiceMock = new Mock<IResourceService>();
            resourceServiceMock.Setup(c => c.GetAllResources()).Returns(resources);
